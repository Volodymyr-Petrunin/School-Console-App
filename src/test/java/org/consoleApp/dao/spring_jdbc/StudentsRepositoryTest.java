...
class StudentsRepositoryTest extends AbstractContainerBaseTest {
    ...
    private static final CleanupAndFillData cleanupAndFillData = new CleanupAndFillData(dataSource);
    ...
    void cleanupAndFillData(){
        ...
        studentsRepository.insertBatch(expectedStudents);
    }

    @Test
    void testFindAll_ShouldFindAllStudents() {
        actual = studentsRepository.findAll();
    ...
    void testFindById_ShouldFindCorrectStudentById() {
        Optional<Student> findStudent = studentsRepository.findById(1);
    ...
    void testInsertBatch_ShouldInsertBatchOfStudents_AndReturnCorrectListOfStudents() {
        ...
        studentsRepository.insertBatch(studentsBatch);
        actual = studentsRepository.findAll();
    ...
    void testUpdate_ShouldUpdateFirstStudentsFromExpectedList() {
        ...
        boolean update = studentsRepository.update(student);
        actual = studentsRepository.findAll();
    ...
    void testDelete_ShouldRemoveFirstStudentFromDB() {
        boolean delete = studentsRepository.delete(expectedStudents.get(0));
        actual = studentsRepository.findAll();
    ...
    void testDeleteByStudentId_ShouldRemoveStudentFromDBbyID_AndReturnCorrectList(){
        boolean delete = studentsRepository.deleteByStudentId(1);
    ...
    void testFindByFirstName_ShouldFindCorrectStudentByName_AndReturnCorrectList(){
        ...
        actual = studentsRepository.findByFirstName(firstName);
    ...
    void testEnrollStudentInCourse_ShouldEnrollStudentToCourse_AndReturnCorrectCourseList(){
        ...
        for (Student student : expectedStudents){
            boolean enroll = studentsRepository.enrollStudentInCourse(student.getId(), courseID);
            assertTrue(enroll);
    ...
    void testRemoveStudentFromCourse_ShouldRemoveStudentFromCourse(){
        ...
        for (Course course : findAllCourses()){
            studentsRepository.enrollStudentInCourse(studentID, course.getId());
        }

        boolean remove = studentsRepository.removeStudentFromCourse(studentID, courseID);
        assertTrue(remove);
    ...
    void testFindStudentsByCourseName_ShouldFindCorrectStudentsByCourseName_AndReturnCorrectStudentsList(){
        ...
        for (Student student : expected){
            studentsRepository.enrollStudentInCourse(student.getId(), 1);
        }

        actual = studentsRepository.findStudentsByCourseName(courseName);
